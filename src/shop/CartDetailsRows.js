...
export class CartDetailsRows extends Component {
    ...
    render() {
        if(!this.props.cart || this.props.cart.lenght === 0){
            return <tr>
                <td colSpan="5">Your Cart is empty</td>
            </tr>
        } else {
            return <React.Fragment>
                { this.props.cart.map(item => <tr key={ item.product.id }>
                    <td>
                        <input type="number" value={ item.quantity } onChange= { (ev) => this.handleChange(item.product, ev) } />
                    </td>
                    <td>{ item.product.name }</td>
                    <td>{ item.product.price.toFixed(2) }</td>
                    <td> <button className="btn btn-sm btn-danger" onClick={ () => this.props.removeFromCart(item.product)}>Remove</button></td>
                </tr>
                )}
                <tr>
